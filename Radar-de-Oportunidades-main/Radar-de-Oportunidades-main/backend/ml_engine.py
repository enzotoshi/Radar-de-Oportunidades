"""
Motor de ML para o Radar de Oportunidades.
Combina dados simulados com análise real de mercado quando disponível.
"""
import numpy as np
...
def calculate_opportunity_score(region_id: str, business_id: str, budget: float) -> Dict:
    ...
    AGORA USA DADOS REAIS DE MERCADO quando Google Maps API está configurado!
    ...
    try:
        ...
        real_analysis = analyzer.analyze_location(
            lat=region["lat"],
            lng=region["lng"],
        ...
        print(f"Análise real não disponível, usando dados simulados: {e}")
        real_analysis = None

    # ── 1. Score de Concorrência (menor concorrência = melhor) ──
    if real_analysis and real_analysis.get("data_source") != "Simulado (Google Maps API não configurado)":
        # USA DADOS REAIS da análise
        real_competition = real_analysis["competition"]
        # Densidade real: quanto menor, melhor
        density_factor = max(0, 100 - (real_competition["density_per_km2"] * 5))
        # Rating médio dos concorrentes: se for alto, mercado é bom mas competitivo
        rating_factor = real_competition.get("average_rating", 3.5) * 20
        competition_score = (density_factor * 0.7 + rating_factor * 0.3)
    else:
    ...
    # ── 5. Score de Fluxo Urbano (agora com dados reais de infraestrutura) ──
    if real_analysis and real_analysis.get("data_source") != "Simulado (Google Maps API não configurado)":
        # USA INFRAESTRUTURA REAL
        infrastructure_score = real_analysis["infrastructure"]["infrastructure_score"]
        flow_score = infrastructure_score
    else:
        # Fallback
    ...
    # ── 7. NOVO: Score de Mobilidade (dados reais) ──
    if real_analysis and real_analysis.get("data_source") != "Simulado (Google Maps API não configurado)":
        mobility_score = real_analysis["mobility"]["mobility_score"]
    else:
        mobility_score = region["urban_flow"] * 8  # Fallback

    # ── Score Final Ponderado (ajustado com mobilidade) ──
    weights = {
        "competition": 0.25,
        "demographics": 0.18,
        "income": 0.18,
    ...
    # Prepara métricas detalhadas
    metrics = {
        "competition": {
            "value": round(competition_score, 1),
            "label": "Concorrência",
            "description": f"Densidade de concorrência: {region['competition_density']}/10" if not real_analysis 
                else f"Concorrentes reais: {real_analysis['competition']['total_competitors']} ({real_analysis['competition']['competition_level']})",
        },
        "demographics": {
        ...
        "urban_flow": {
            ...
            "label": "Fluxo Urbano" if not real_analysis else "Infraestrutura Real",
            "description": f"Circulação de pessoas: {region['urban_flow']}/10" if not real_analysis
                else f"Facilidades na área: {real_analysis['infrastructure']['total_facilities']}",
        },
        "budget": {
        ...
        "mobility": {
            "value": round(mobility_score, 1),
            "label": "Mobilidade" if real_analysis else "Fluxo",
            "description": "Transporte público e acessibilidade" if real_analysis 
                else "Estimativa de mobilidade",
        },
    }
    ...
    # Adiciona análise real se disponível
    if real_analysis:
        result["real_market_data"] = real_analysis
        result["data_source"] = real_analysis.get("data_source", "Real Data")
    else:
        result["data_source"] = "Dados Simulados"
