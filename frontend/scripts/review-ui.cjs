...
async function main() {
  ...
  try {
    ...
    assert.deepEqual(calls.find(call => call.path.endsWith('/analyze-with-ai')).body, { address: location.address, business_type: 'cafeteria', lat: location.lat, lng: location.lng, budget: 125000 })
