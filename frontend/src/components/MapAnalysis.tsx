...
export default function MapAnalysis({ selectedRegion, setSelectedRegion, selectedBusiness, setSelectedBusiness, analysisResult, setAnalysisResult, onGoToInvestor }: Props) {
  ...
  return <div className="territory-page">
    ...
    <div className="atlas-workspace" data-has-result={Boolean(analysisResult)}>
      ...
      {!analysisResult && <aside className="analysis-guide" aria-label="Guia de evidências"><span className="section-kicker">Como ler o Radar</span><h2>Da busca à decisão</h2><ol><li><strong>1</strong><span>Escolha um endereço verificado.</span></li><li><strong>2</strong><span>Consulte sinais territoriais com fonte.</span></li><li><strong>3</strong><span>Compare cenários antes de decidir.</span></li></ol><p>Dados ausentes permanecem identificados — nunca são substituídos por estimativas silenciosas.</p></aside>}
    </div>
    {analysisResult && <div id="analysis-insights" hidden={!insightsOpen}><AnalysisInsights result={analysisResult} onClear={() => { invalidateAnalysis(); setError(null) }} onGoToInvestor={onGoToInvestor} /></div>}
  </div>
}
